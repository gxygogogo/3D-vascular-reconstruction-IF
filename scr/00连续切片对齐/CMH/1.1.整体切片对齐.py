...
## 预设路径
slide_src_dir = "/public3/Xinyu/3D_tissue/IF/MIF_CMH"
results_dst_dir = "/public3/Xinyu/3D_tissue/IF/01.mIHC_rigid_registration/MIF_CMH"
registered_slide_dst_dir = "/public3/Xinyu/3D_tissue/IF/01.mIHC_rigid_registration/MIF_CMH/registered_slides"

os.makedirs(results_dst_dir, exist_ok = True)
os.makedirs(registered_slide_dst_dir, exist_ok = True)

sorted_img_f_list = ['图像_CMH_' + str(x) + '_CD31_LYVE1' for x in range(2, 102)]
...
## 导入对齐对象
registrar = registration.load_registrar(f'/public3/Xinyu/3D_tissue/IF/01.mIHC_rigid_registration/MIF_CMH/MIF_CMH/data/MIF_CMH_registrar.pickle')

## 逐张导出
pat = re.compile(r'CMH_\d+')
for i in sorted_img_f_list[42:]:
    print(i)
    slide_f = f'/public3/Xinyu/3D_tissue/IF/MIF_CMH/{i}.vsi'
    m = pat.search(i)
    base = m.group(0)
