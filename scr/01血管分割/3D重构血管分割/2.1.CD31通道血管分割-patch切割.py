...
sample_list = [f.split('_CD31_mask.tif')[0] for f in os.listdir('/public3/Xinyu/3D_tissue/IF/03.mIHC_vascular_segmentation/CMH/mask') if f.endswith('_CD31_mask.tif')]
#sample = 'HJZ_5-00004-02772-17487'

for sample in tqdm(sample_list, desc="批量处理进度"):
    folder_sample = re.sub(r'_\d+', '', sample)
    # ================= 修改区 =================
    # 原图路径 (CD31 单通道)
    img_path  = f'/public3/Xinyu/3D_tissue/IF/02.mIHC_ROI_RigidMicro_registration/{folder_sample}/Micro-registration/micro_registered_slides_CD31/{sample}_uint16_enhanced.tif' 

    # 掩码图路径
    mask_path = f'/public3/Xinyu/3D_tissue/IF/03.mIHC_vascular_segmentation/CMH/mask/{sample}_CD31_mask.tif' 
    # ==========================================

    # 输出目录
    base_out = f'/public3/Xinyu/3D_tissue/IF/03.mIHC_vascular_segmentation/CMH/patch/{sample}_{patch_size}_CD31_Balanced'
