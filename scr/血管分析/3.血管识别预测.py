...
if __name__ == "__main__":
    file_list = ['PSKO2_CD31_1', 'ETPSWT5_CD31_1', 'ETKO2_CD31_1', 'ETPSKO3_CD31_1']

    for i in file_list:
        print(i)
        # 1. 输入图片路径 (CD31 单通道大图)
        input_image_path = f"/public3/Xinyu/3D_tissue/IF/Vascular_stat/CD31_tif/{i}.tif"
        
        # 2. 模型权重路径 (你训练好的 .pth 文件)
        model_path = "/public3/Xinyu/3D_tissue/IF/Vascular_stat/models/best_cd31_unet.pth"
        # pat = re.compile(r'HJZ_\d+-\d+-\d+-\d+')
        # m = pat.search(i)
        # base = m.group(0)
        
        # 3. 输出路径
        output_mask_path = f"/public3/Xinyu/3D_tissue/IF/Vascular_stat/prediction/{i}_pred_mask.tif"
